...
test('creates user and logs-in', async () => {
  ...
  // Sign up
  await click(await screen.findByRole('link', { name: 'auth:menu.signUp' }, { timeout: 5000 }));
  ...
  // Log in
  await screen.findByText('auth:signUp.success', undefined, { timeout: 5000 });
  ...
  // Log out
  await click(await screen.findByText(email, undefined, { timeout: 5000 }));
